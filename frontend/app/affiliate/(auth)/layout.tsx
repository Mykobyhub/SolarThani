...
// Cloned from app/(auth)/layout.tsx — the installer/admin auth-card chrome —
// scoped to /affiliate/login, /affiliate/forgot-password, /affiliate/reset-password
// only (the affiliate role's own auth surfaces, separate cookie/session).
...
export default async function AffiliateAuthLayout({ children }: { children: React.ReactNode }) {
          ...
          <Link href="/affiliate" className="inline-block">
            {site.logo ? (
              ...
              <div className="inline-flex items-center gap-2.5">
                ...
                <span className="font-black text-xl text-[var(--color-text)]">{site.name} Affiliate</span>
              </div>
            )}
          </Link>
        </div>
