...
export default function AffiliateSignupForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.success) {
        setSuccess(data.message || 'สมัครสำเร็จ! กรุณาตรวจสอบอีเมลเพื่อยืนยันบัญชี');
  ...
  if (success) {
    return (
      <div className="text-center">
        ...
        <Link href="/affiliate/login" className="btn btn-primary">
          ไปหน้าเข้าสู่ระบบ
        </Link>
      </div>
    );
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="form-group">
          ...
          <input type="text" className="form-input" placeholder="ชื่อของคุณ" value={form.name} onChange={update('name')} required />
        </div>
        ...
        <div className="flex items-start gap-3 pt-1">
          ...
          />
          <label htmlFor="aff-agree-terms" className="text-sm leading-snug cursor-pointer" style={{ color: 'var(--color-muted)' }}>
            ฉันได้อ่านและยอมรับ{' '}
            <a
              href="/terms"
              target="_blank"
            ...
            ของโปรแกรม Affiliate <span className="text-red-500">*</span>
          </label>
        </div>

        <button type="submit" disabled={loading || !agreed} className="btn btn-primary w-full justify-center mt-2">
          {loading ? 'กำลังสมัคร...' : 'สมัครเข้าร่วมโปรแกรม'}
        </button>
      </form>

      <p className="text-center text-sm text-[var(--color-muted)] mt-6">
        มีบัญชี Affiliate อยู่แล้ว?{' '}
        <Link href="/affiliate/login" className="text-[var(--color-primary)] font-semibold hover:underline">
          เข้าสู่ระบบ
        </Link>
      </p>
    </>
