...
export async function POST(req: NextRequest) {
  ...
  const url = await saveFile(file, 'banners', `installer-banner-${session.id}-${Date.now()}`);
  await db.prepare('UPDATE installers SET banner_image = ? WHERE id = ?').run(url, session.id);

  return NextResponse.json({ success: true, banner_image: url });
}
