...
export async function POST(req: NextRequest) {
  try {
    ...
    const affiliate = await db.prepare('SELECT id FROM affiliates WHERE email = ?').get(reset.email);
    if (!affiliate)
      return NextResponse.json({ success: false, error: 'ลิงก์ไม่ถูกต้องหรือหมดอายุแล้ว' }, { status: 400 });

    const password_hash = await bcrypt.hash(password, 12);
    await db.prepare('UPDATE affiliates SET password_hash = ?, updated_at = NOW() WHERE email = ?').run(password_hash, reset.email);
    ...
    console.error('Affiliate reset password error:', err);
